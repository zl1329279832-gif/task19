...
self.onmessage = function(e) {
  ...
  switch (action) {
    case 'autoSchedule':
      ...
      self.postMessage({ action: 'autoScheduleResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
    case 'detectConflicts':
      result = detectConflicts(data);
      self.postMessage({ action: 'conflictResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
    case 'analyzeRisks':
      result = analyzeRisks(data);
      self.postMessage({ action: 'riskResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
    case 'insertOrder':
      result = insertOrder(data);
      self.postMessage({ action: 'insertResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
    case 'recalcAfterDrag':
      result = recalcAfterDrag(data);
      self.postMessage({ action: 'recalcResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
    case 'scenarioCalculate':
      result = calculateScenario(data);
      self.postMessage({ action: 'scenarioResult', data: result, requestVersion, scenarioId, scenarioVersion });
      break;
  }
};
